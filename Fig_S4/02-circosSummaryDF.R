...
gps = t1$abund_data$Sample
taxs = t1$abund_data$Taxonomy
abund = t1$abund_data$Abundance
newdf = data.frame(abund, taxs, gps)

newdf = newdf[newdf$taxs %in% t1$use_taxanames, ]
...
df.orderCols = data.frame(Od = 30:22, 
                          samCols = c(cCols, cCols, mCols, cCols, mCols,
                                      cCols, mCols, cCols, mCols))
newdf.re = cbind(df.orderCols, newdf.re)


write.table(df.head, '03-CircosDF.txt', append = F, quote = F, sep = '\t',
            row.names = F,col.names = F)

write.table(newdf.re, '03-CircosDF.txt', append = T, quote = F, sep = '\t',
            row.names = F,col.names = F)

# '#E69F00' --C
# '#0072B2' --M

# submit to http://mkweb.bcgsc.ca/tableviewer/
