...
randomDF = read.table('05-simuTargetDelt.txt', header = T, sep = '\t', stringsAsFactors = F)
...
ptarget = ggplot(randomDF, aes(x = network, y = remain.mean, fill = Group)) +
  geom_col() +
...
# ggsave('08-SimulationTarget.pdf', ptarget, units = 'in', width = 3, height = 2.5)
...
write.table(data.frame(ComparePairs = cp, statisticD, Pvalue), '07-WMtestTarget.txt', 
...
randomDF = read.table('05-simuRandomDelt.txt', header = T, sep = '\t', stringsAsFactors = F)
...
prandom = ggplot(randomDF, aes(x = network, y = remain.mean, fill = Group)) +
  geom_col() +
  geom_errorbar(aes(ymin=remain.mean-remain.sd, ymax=remain.mean+remain.sd), 
                width=.2, position=position_dodge(.9)) + 
  facet_grid(.~TimeGP, space = 'free', scales = 'free_x') +
...
# ggsave('08-SimulationRandom.pdf', prandom, units = 'in', width = 3, height = 2.5)
...
write.table(data.frame(ComparePairs = cp, statisticD, Pvalue), '07-WMtestRandom.txt', 
            append = F, quote = F, sep = '\t', row.names = F, col.names = T)

##########################################################################
##########################################################################

ggsave('08-SimulationRobust.pdf', grid.arrange(prandom, ptarget, nrow = 1), 
