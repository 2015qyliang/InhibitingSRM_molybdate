...
diybox = function(v.meas) {
  ...
  p = ggplot(alphaDF, aes(x = Group, y = Value, fill = Group)) + 
    ...
    facet_grid(facets = .~Measure, rows = 1, scales = 'free_y') +
    ...
    theme(panel.grid = element_blank(), 
          ...
          axis.text.x = element_text(size = 8, hjust = 0.5, vjust = 0.5, angle = 90, color = 'black'),
...
alphaDF.raw = t1$alpha_data[, c('Measure', 'Value', 'SampleID', 'Group')]

##############################################################################

alphaDF.output = alphaDF.raw
alphaDF.output$Value = round(alphaDF.output$Value, digits = 3)
write.table(alphaDF.output, '03-AlphaDF.txt', 
...
for (index.alpha in vec.Measure) {
  ...
  write.table(data.frame(Compares = rownames(posthoc$Group), posthoc$Group), 
              paste0('04-main_ANOVA_TurkeyHSDtest_', index.alpha, '.txt'), 
...
ggsave("06-Alpha_supplement.pdf", p, units = "in", height = 6, width = 4)
...
for (index.alpha in vec.Measure) {
  ...
  posthoc = TukeyHSD(aov(Value ~ Group, data = df1), 'Group', conf.level=0.95)
  write.table(data.frame(Compares = rownames(posthoc$Group), posthoc$Group), 
              paste0('07-sup_ANOVA_TurkeyHSDtest_', index.alpha, '.txt'), 
              append = F, quote = F, sep = '\t', row.names = F, col.names = T)
}
