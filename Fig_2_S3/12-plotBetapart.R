...
# https://www.nature.com/articles/s41396-019-0396-x
# Forfeiting the priority effect: turnover defines biofilm community succession
# The ISME Journal volume 13, pages1865–1877 (2019)
...
p1 = all.melt %>% plot_dm(ylab = "Incubation Days (Control)") + scale_fill_viridis(limits=c(0,1)) 
...
p2 = all.melt %>% plot_dm(ylab = "Incubation Days (Treat)") + scale_fill_viridis(limits=c(0,1))

# ggsave('13-betapart_M.pdf', p2, width = 6, height = 3)

# set the layout of result
pdf(width = 6.5, height = 5, file = '13-betapart.pdf' )
grid.newpage()  
...
write.table(newdf, '14-adnois.txt', append = F, quote = F, sep = '\t', 
