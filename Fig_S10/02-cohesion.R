...
# cohesion compulating 
# Reference:
# https://github.com/cherren8/Cohesion
# Cohesion: a method for quantifying the connectivity of microbial communities
# ISME J. 2017 Nov;11(11):2426-2438
...
refCohesioin = function(otudf.sub){
  ...
  # b = read.table(fn, header = T, sep = '\t', row.names = 1)
  b = otudf.sub
  
  # Read in custom correlation matrix, if desired. Must set "use.custom.cors" to TRUE
  if(use.custom.cors == T) {
    # custom.cor.mat = read.table(fn, header = T, sep = '\t',row.names = 1)
    custom.cor.mat = otudf.sub
...
otudf = t(read.table('01-RelTable.txt', header = T, sep = '\t', row.names = 1))
gps = c('T00C', 'T05C', 'T05M', 'T12C', 'T12M', 'T21C', 'T21M', 'T30C', 'T30M')

for (gp in gps) {
  subdf = otudf[str_detect(rownames(otudf), gp), ]
  subdf = subdf[, colSums(subdf) != 0]
  coh = refCohesioin(as.matrix(subdf))
  saveRDS(coh, paste0('03-cohesion_D', substr(gp, 2, 4), '.rds'))
}
